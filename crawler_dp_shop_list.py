...
from util.prase_dp_shop_list import PraseList
import logger
...
class Crawler():
    ...
    def _start_proxy_check_worker(self):
        worker = ProxyCheckWorker(self.proxy_queue,PraseList.check_proxy_available)
    ...
    def _start_monitor(self):
        worker = QueueMonitor(self,DataService.add_dp_list_job,LINKS_QUEUE_MIN_SIZE,DOWNLOAD_WORKERS_SIZE)
        #worker.setDaemon(True)
        worker.start()
        self.threads.append(worker)
    ...
    def _start_new_extractor(self):
        worker = ExtractorWorker(self.pages_queue,PraseList.if_has_data,PraseList.parse_content,DataService.update_dp_list_task)
        #worker.setDaemon(True)
        worker.start()
