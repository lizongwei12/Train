...
from util.prase_price_tc import PrasePrice
import logger
...
class Crawler():
    ...
    def _start_proxy_check_worker(self):
        worker = ProxyCheckWorker(self.proxy_queue)
    ...
    def _start_monitor(self):
        worker = QueueMonitor(self,DataService.add_station_job,LINKS_QUEUE_MIN_SIZE,DOWNLOAD_WORKERS_SIZE)
        #worker.setDaemon(True)
        worker.start()
        self.threads.append(worker)
    ...
    def _start_new_proxy_checker(self):
        worker = ProxyCheckWorker(self.proxy_queue,PrasePrice.check_proxy_available)
        #worker.setDaemon(True)
        worker.start()
        self.threads.append(worker)
    ...
    def _start_new_extractor(self):
        worker = ExtractorWorker(self.pages_queue,PrasePrice.if_has_data,PrasePrice.parse_content,DataService.update_task)
        #worker.setDaemon(True)
        worker.start()
